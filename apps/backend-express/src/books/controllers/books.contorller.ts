...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const books: Book[] = serviceResponse as Book[];
    return res.status(200).json(books);
  } catch (error) {
    return res.status(500).send({ message: 'DB down' });
  }
});
...
router.post('/', async (req: Request, res: Response) => {
  if (checkIfCreateBookDto(req?.body)) {
    ...
    try {
      ...
      if (serviceResponse.acknowledged) {
        const _id = serviceResponse.insertedId.toString();
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (serviceResponse.deletedCount === 1) {
